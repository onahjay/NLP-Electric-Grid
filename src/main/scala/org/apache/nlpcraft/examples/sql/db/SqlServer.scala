...
object SqlServer extends LazyLogging {
    private final val H2_PORT: Int = 5435
    ...
    private final val SRV_PARAMS = Seq(
        "-baseDir", H2_BASEDIR,
        "-pgPort", H2_PORT.toString,
        "-pgAllowOthers"
    )

    private final val INIT_FILE = "src/main/scala/org/apache/nlpcraft/examples/sql/db/dump.sql"

    final val H2_URL: String = s"jdbc:h2:~/test;MODE=PostgreSQL"
    ...
    def start(): Unit = {
        ...
        srv = Server.createPgServer(SRV_PARAMS:_*).start
