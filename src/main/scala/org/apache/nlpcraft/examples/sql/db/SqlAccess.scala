...
import java.sql.{Connection, PreparedStatement, SQLException, DriverManager}
    ...
    def select(qry: SqlQuery, logResult: Boolean): SqlResult = {
        def getConnection: Connection = {
            try {
                ...
                DriverManager.getConnection("jdbc:postgresql://localhost:5432/reu_test", "postgres", "password")
            }catch {
              case e: Exception ⇒
                e.printStackTrace()
                throw e
