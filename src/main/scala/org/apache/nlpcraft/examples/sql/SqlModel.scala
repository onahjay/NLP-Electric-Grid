...
class SqlModel extends NCModelFileAdapter("org/apache/nlpcraft/examples/sql/sql_model.yaml") with LazyLogging {
    ...
    @NCIntent(
        ...
        "term(cols)={id == 'col:date' || id == 'col:num' || id == 'col:varchar' || groups @@ 'column'}[0,7] " +
    ...
    @NCIntent(
        ...
        "term(tbls)={id == 'tbl:radience_dynamic_nodesgis'}[0,7] " +
        "term(cols)={id == 'col:radience_dynamic_nodesgis_'}[0,7] " +
        "term(condNums)={id == 'condition:num'}[0,7] " +
        "term(condVals)={id == 'condition:value'}[0,7] " +
        "term(condDates)={id == 'condition:date'}[0,7] " +
        "term(condFreeDate)={id == 'nlpcraft:date'}? " +
    ...
    def onCustomSortReport(
        ...
        val ordersFreightColSort: NCSqlSort =
            new NCSqlSort {
                override def getColumn: NCSqlColumn = SCHEMA.getTables.asScala.find(_.getTable == "resiliency_scores").
                    getOrElse(throw new RuntimeException(s"Table `resiliency_scores` not found.")).
                    getColumns.asScala.find(_.getColumn == "res_val").
                    getOrElse(throw new RuntimeException(s"Column `resiliency_scores.res_val` not found."))
...
/*
    /**
     * COPIED FROM onCustomSortReport to test if same def but diff cols works
     *
     * Modified version of main generic intent, which uses implicit sort element definition.
     * It is developed as an example of way by which this model can be extended to support more
     * complicated questions comparing to a generic case.
     ...
     */
    @NCIntent(
        "intent=customSortReport2 conv=true " +
          "term(sort)={id == 'sort:best' || id == 'sort:worst'} " +
          "term(tbls)={groups @@ 'table'}[0,7] " +
    ...
    def onCustomSortReport2(
        ...
        val ordersFreightColSort: NCSqlSort =
            new NCSqlSort {
                override def getColumn: NCSqlColumn = SCHEMA.getTables.asScala.find(_.getTable == "load_details").
                  getOrElse(throw new RuntimeException(s"Table `resiliency_scores` not found.")).
                  getColumns.asScala.find(_.getColumn == "l_kv").
                  getOrElse(throw new RuntimeException(s"Column `resiliency_scores.res_val` not found."))
 ...
 */
     ...
     */
    @NCIntent(
        "intent=customSortReport3 conv=true " +
          "term(sort)={id == 'sort:best' || id == 'sort:worst'} " +
          "term(tbls)={id == 'tbl:economic_mode'}[0,7] " +
          "term(cols)={id == 'col:date' || id == 'col:num' || id == 'col:varchar' && id != 'col:resiliency_scores_res_val'}[0,7] " +
          "term(condNums)={id == 'condition:num'}[0,7] " +
          "term(condVals)={id == 'condition:value'}[0,7] " +
          "term(condDates)={id == 'condition:date'}[0,7] " +
          "term(condFreeDate)={id == 'nlpcraft:date'}? " +
    ...
    def onCustomSortReport3(
        ...
        val ordersFreightColSort: NCSqlSort =
            new NCSqlSort {
                override def getColumn: NCSqlColumn = SCHEMA.getTables.asScala.find(_.getTable == "economic_mode").
                  getOrElse(throw new RuntimeException(s"Table `resiliency_scores` not found.")).
                  getColumns.asScala.find(_.getColumn == "eco_res_val").
                  getOrElse(throw new RuntimeException(s"Column `resiliency_scores.res_val` not found."))
